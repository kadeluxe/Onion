...
//Stole these macros from katatem, I liked what they look like.

#define GEN_CALL(...) (this, ##__VA_ARGS__)
#define VCALL(idx, T) VMT::getvfunc<T>(this, idx)GEN_CALL
#define VVALUE(type, idx, T) (type)VCALL(idx,T)
#define VRESULT(type, idx, T) return VVALUE(type,idx,T)
